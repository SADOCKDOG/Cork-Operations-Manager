...
main.innerHTML = `<div class="card" style="animation: fadeInUp 0.4s ease;"><div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"><h3>Gestión de Zonas</h3><div data-action="App.openManualZonas" style="cursor:pointer; color:var(--accent); font-weight:bold; font-size:0.85rem;">Ayuda → <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg></div></div><div class="grid-2" style="gap:10px;"><button class="btn btn-primary" data-route="/zona/nueva">Nueva Zona</button><button class="btn btn-secondary" data-route="/importar-pdf">Importar PDF</button></div></div>${stats.map((z, i) => `<div class="card" data-route="/zona/${z.id}" style="cursor:pointer; border-top: 5px solid var(--accent); padding: 25px; animation: fadeInUp 0.4s ease; animation-delay: ${i * 0.05}s;"><h3 style="text-align:center; color: #fff; font-size: 1.4rem; margin-bottom: 20px; border:none;">${App.escapeHtml(z.nombre)}</h3><div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; padding-top: 5px; text-align:center;"><div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">1ª CAL</div><div class="bg-pill-lime text-lime" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${z.totalesPorCalidad.primera.quintales.toFixed(2)}</div></div><div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">BORNIZO</div><div class="bg-pill-gold text-gold" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${z.totalesPorCalidad.bornizo.quintales.toFixed(2)}</div></div><div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">REFUGO</div><div class="bg-pill-red text-red" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${z.totalesPorCalidad.refugo.quintales.toFixed(2)}</div></div></div></div>`).join('')}`;
},
...
<button class="btn btn-secondary mt-1" data-route="/zona/${z.id}/editar">Editar Zona</button>
</div>
...
<div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; padding-top: 5px; text-align:center;">
<div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">1ª CAL</div><div class="bg-pill-lime text-lime" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${t.primera.toFixed(2)}</div></div>
<div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">BORNIZO</div><div class="bg-pill-gold text-gold" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${t.bornizo.toFixed(2)}</div></div>
<div><div class="text-grey" style="font-size: 0.7rem; font-weight: 700; margin-bottom: 6px;">REFUGO</div><div class="bg-pill-red text-red" style="font-weight: 800; padding: 4px 8px; border-radius: 6px; display: inline-block;">${t.refugo.toFixed(2)}</div></div>
</div>
</div>
...
${isEdit ? `<button type="button" class="btn btn-danger mt-1" data-action="App._deleteZona" data-id="${id}">Eliminar Zona</button>` : ''}
...
App.toast(`${zonesToSave.length} zonas añadidascorrectamente`);
location.hash = '/zonas';
};
},
