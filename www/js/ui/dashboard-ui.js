...
export const DashboardUI = {
    ...
    async renderUltimasPesadas() {
        ...
        for (const p of recent) {
            ...
            h += `<div class="pesada-card" style="--card-color: ${col}; cursor:pointer;" data-route="/pesada/${p.id}/editar"><div class="pesada-card-content"><div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;"><div class="pesada-saca-badge" style="font-size:0.8rem; padding:4px 8px;">SACA #${p.saca}</div><strong style="color: ${col}; font-size: 1.1rem;">${em} ${cal}</strong></div><div style="display:flex; justify-content:space-between; color:#fff; align-items:center;"><span>${fh}</span><strong style="font-size:1.2rem;">${p.kg.toFixed(1)} kg</strong></div></div></div>`;
        }
